...
import SecureAreaPage from '../pageobjects/secureArea.page.js'
...
describe('TC-01 - Should log in when valid username and valid password are provided', () => {
    ...
    it('Alert box should have proper text', async () => {
        await expect(SecureAreaPage.flashAlert).toHaveTextContaining('You logged into a secure area!')
    })
    it('Header should have proper text', async () => {
        await expect(SecureAreaPage.header).toHaveTextContaining('Secure Area')
    })
    it('Subeader should have proper text', async () => {
        await expect(SecureAreaPage.subheader).toHaveTextContaining('Welcome to the Secure Area. When you are done click logout below.')
    })
})
...
describe('TC-02 - Should logout when Logout button is clicked', () => {
    ...
    it('Alert box should have proper text', async() => {
        await expect(SecureAreaPage.flashAlert).toHaveTextContaining('You logged out of the secure area!')
    })
})
...
describe('TC-03 - Should not log in when valid username and invalid password are provided', () => {
    ...
    it('Alert box should have proper text', async() => {
        await expect(SecureAreaPage.flashAlert).toHaveTextContaining('Your password is invalid!')
    })
})
...
describe('TC-04 - Should not log in when invalid username and valid password are provided', () => {
    ...
    it('Alert box should have proper text', async() => {
        await expect(SecureAreaPage.flashAlert).toHaveTextContaining('Your username is invalid!')
    })
})
